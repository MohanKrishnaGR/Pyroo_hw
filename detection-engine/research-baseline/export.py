...
def run(
    ...
    im = torch.zeros(batch_size, ch, *imgsz).to(
        device
    ...
    if engine:  # TensorRT required before ONNX
        f[1], _ = export_engine(
            model, im, file, half, dynamic, simplify, workspace, verbose, cache
        )
...
def parse_opt(known=False):
    ...
    parser.add_argument(
        "--cache", type=str, default="", help="TensorRT: timing cache file path"
    )
